...
def main(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        clf.fit(X, y)
        best_index = best_low_complexity(clf.cv_results_)
        ...
        median_split_index = np.argsort(
            [
                clf.cv_results_[f'split{i}_test_accuracy'][best_index]
        ...
        num_pos_classified_as_uncertain = np.asarray(
            [
                np.sum(
                    (svm_decision_threshold < y_cv_pred)
                    & (y_cv_pred <= possible_threshold)
                    & (y == 1)
                )
        ...
        num_neg_classified_as_uncertain = np.asarray(
            [
                np.sum(
                    (possible_threshold <= y_cv_pred)
                    & (y_cv_pred < svm_decision_threshold)
                    & (y == 0)
                )
        ...
        threshold_waypoints["threshold_values_1p0"][1] = np.percentile(
            y_cv_pred[
                (y == 1) & (y_cv_pred >= possible_threshold_points_pos.iloc[threshold_0p5_pos])
            ],
            95,
        )
        threshold_waypoints["threshold_values_1p0"][0] = np.percentile(
            y_cv_pred[
                (y == 0) & (y_cv_pred <= possible_threshold_points_neg.iloc[threshold_0p5_neg])
            ],
            5,
        )
