...
def main(
    ...
    median_split_index = np.argsort(
        [clf.cv_results_[f'split{i}_test_accuracy'][best_index] for i in range(cv.get_n_splits())]
    )[cv.get_n_splits() // 2]
    ...
    pickle_dump(
        dict(clustering=pipeline.named_steps['clustering']), training_dir / 'DoQA_config_debug.pkl'
    )
